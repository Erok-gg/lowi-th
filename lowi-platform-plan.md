...
| **3a** | **Migration vitrine partie 1** ✅ | ✅ Route group `(public)/`, layout (LangProvider + LowiNav + LowiHalo + LowiFooter), `page.tsx` (landing : hero, PriceTicker, trust band, carousel 6 cartes), `a-propos/page.tsx`. `public.css` + `landing.css` + `a-propos.css`. Font MCTen dans `public/fonts/`. Suppression des anciens `app/page.tsx` et `(dashboard)/page.tsx` (conflit résolu). Build ✅ `/` et `/a-propos` en Static. | ✅ `next build` OK. `GET /` 200. Snapshot complet. Aucune régression dashboard. |
